...
void Logger::Impl::formatTime() {
    ...
    // 写入此线程存储的时间buf中
    snprintf(t_time, sizeof(t_time), "%4d/%02d/%02d > %02d:%02d:%02d:", tm_time->tm_year + 1900,
    ...
    stream_ << GeneralTemplate(t_time, 22) << GeneralTemplate(buf, 7);
}
...
void Logger::Impl::formatTime2() {
    ...
    if (seconds != t_lastSecond) {
        ...
        int len = snprintf(t_time, sizeof(t_time), "%4d%02d%02d %02d:%02d:%02d", dt.year,
...
void Logger::Impl::finish() {
    stream_ << " - " << GeneralTemplate(basename_.data_, basename_.size_) << ':' << line_ << ":"
            << GeneralTemplate(CurrentThread::tidString(), CurrentThread::tidStringLength()) << '\n';
